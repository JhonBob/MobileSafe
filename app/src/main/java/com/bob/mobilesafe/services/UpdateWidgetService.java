...
import com.bob.mobilesafe.receives.MyWidget;
    ...
    public void onCreate() {
        ...
        task = new TimerTask() {
            ...
            public void run() {
                ...
                views.setTextViewText(R.id.process_count, "运行中的进程："+SystemInfoUtils.getRunningProcessCount(getApplicationContext()));
                String availsize = Formatter.formatFileSize(getApplicationContext(), SystemInfoUtils.getAvaliMem(getApplicationContext()));
                views.setTextViewText(R.id.process_memory, "可用内存："+availsize);
                //由另外一个进程执行的动作,由桌面发送一个广播
                Intent intent = new Intent();//自定义的广播
                intent.setAction("com.bob.mobilesafe.killall");
